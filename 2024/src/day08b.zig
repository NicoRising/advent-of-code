        ...
        while (first_idx < locations.items.len) : (first_idx += 1) {
            ...
            while (second_idx < locations.items.len) : (second_idx += 1) {
                if (first_idx != second_idx) {
                    ...
                    while (antinode.x >= 0 and antinode.y >= 0 and antinode.x < width and antinode.y < height) : (antinode = Point{
                        .x = antinode.x + second.x - first.x,
                        .y = antinode.y + second.y - first.y,
                    }) {
