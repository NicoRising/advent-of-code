    ...
    for (0..width) |start_x| {
        for (0..height) |start_y| {
            if (map[start_x][start_y]) |start_dist| {
                const left = @max(start_x, 20) - 20;
                const right = @min(start_x + 20, width - 1);

                const top = @max(start_y, 20) - 20;
                const bottom = @min(start_y + 20, height - 1);
...
test "test day20b" {
    ...
    const result = try solve(input, 72);

    try std.testing.expectEqual(29, result);
}
